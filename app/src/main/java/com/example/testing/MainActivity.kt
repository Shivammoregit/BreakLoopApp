...
class MainActivity : AppCompatActivity() {
    ...
    private fun updateStatsDisplay() {
        try {
            ...
            // Persist today's total for progress report
    ...
    private fun updateSevenDayProgressCard() {
        val container = findViewById<LinearLayout>(R.id.progress7dayContainer)
        val headerText = findViewById<TextView>(R.id.progress7dayHeader)
        val cta = findViewById<View>(R.id.progress7dayCta)
        if (container == null || headerText == null || cta == null) return

        val last7 = UsageUtils.getLastNDaysTotals(this, 7)
        container.removeAllViews()

        // Baseline: first day in the list
        val baseline = last7.firstOrNull()?.second ?: 0
        var improvedDays = 0

        last7.forEach { (day, minutes) ->
            if (minutes < baseline) improvedDays++
            ...
            val dateText = TextView(this).apply {
                text = day.substring(5) // show MM-DD
                textSize = 12f
                setTextColor(ContextCompat.getColor(this@MainActivity, R.color.textPrimary))
                layoutParams = LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 0.2f)
            }
            ...
            val barBg = View(this).apply {
                layoutParams = LinearLayout.LayoutParams(0, 8, 0.6f)
                setBackgroundColor(ContextCompat.getColor(this@MainActivity, R.color.textSecondary))
                alpha = 0.2f
            }

            val maxIn7 = (last7.maxOfOrNull { it.second } ?: 1).coerceAtLeast(1)
            val pct = (minutes.toFloat() / maxIn7 * 100).toInt().coerceIn(1, 100)
            val barFill = View(this).apply {
                layoutParams = LinearLayout.LayoutParams(0, 8, pct / 100f)
        ...
        headerText.text = "📈 7-Day Progress (${improvedDays}/7 better than day 1)"
